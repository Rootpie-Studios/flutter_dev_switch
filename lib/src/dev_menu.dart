...
/// "Refuse writes" switches (see [DevFaults]), then whatever [entries] the
/// app adds. [show] opens it; the [DevShell] calls that from every screen.
...
class DevMenu extends StatelessWidget {
  ...
  static Future<void> show(
    ...
    return showDevSheet<void>(
      context,
      builder: (_) =>
          DevMenu(config: config, entries: entries, strings: strings),
    );
  }
    ...
    builder: (context, _) => DevSheetBody(
      ...
      children: [
        ListTile(
          ...
          onTap: () => closeThen(
            context,
            (root) => ServerPicker.show(root, config: config, strings: strings),
          ),
        ),
        ListTile(
